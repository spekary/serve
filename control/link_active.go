...
// You then alter them directly to set up the attributes that will visually make the link active.
...
// DrawingAttributes is called by the framework to retrieve the tag's attributes at draw time.
...
func init() {
	page.RegisterControl(func() page.ControlI { return new(ActiveLink) })
}
