...
import (
	...
	"github.com/goradd/serve/control/button"
...
func (i *ImageCapture) Init(self page.ControlI, parent page.ControlI, id string) {
	i.Panel.Init(self, parent, id)
	i.Form().AddJavaScriptFile(path.Join(config.AssetPrefix, "goradd", "/js/image-capture.js"), false, nil)
	i.typ = "jpeg"
	i.quality = 0.92

	control.NewCanvas(i, i.canvasID())

	button.NewButton(i, i.captureID()).
		SetText(i.T("New Image"))

	button.NewButton(i, i.switchID()).
		SetDisplay("none").
		SetText(i.T("Switch Camera"))

	i.ErrTextID = i.ID() + "-err"
	et := control.NewPanel(i, i.ErrTextID)
	et.Tag = "p"
	et.SetDisplay("none")
	et.SetText(i.T("This browser or device does not support image capture"))
}
...
func (i *ImageCapture) SetPixelSize(width int, height int) {
	canvas := control.GetCanvas(i, i.canvasID())
...
func (i *ImageCapture) UpdateFormValues(request *page.RequestContext) {
	/*
		Reeimplement using a custom html control and standard control values
		if data := request.CustomControlValue(i.ID(), "data"); data != nil {
			s := data.(string)
...
func GetImageCapture(c page.ControlI, id string) *ImageCapture {
	return c.Form().GetControl(id).(*ImageCapture)
}

func init() {
	page.RegisterControl(func() page.ControlI { return new(ImageCapture) })
}
