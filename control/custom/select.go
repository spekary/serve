...
func RowSelectedEvent() *event.Event {
	return event.NewEvent("gr-rowselected")
}
...
func (t *SelectTable) Init(self page.ControlI, parent page.ControlI, id string) {
	...
	t.Form().AddJavaScriptFile(path.Join(config.AssetPath, "serve", "/js/goradd-scrollIntoView.js"), false, nil)
	t.Form().AddJavaScriptFile(path.Join(config.AssetPath, "serve", "/js/table-select.js"), false, nil)
...
func (t *SelectTable) RowAttributes(row int, data interface{}) (a html5tag.Attributes) {
	...
	if t.RowStyler() != nil {
		a = t.RowStyler().RowAttributes(row, data)
...
func (t *SelectTable) UpdateFormValues(request *page.RequestContext) {
	if data, ok := request.FormValue(t.ID()); ok {
		t.selectedID = data
	}
}
...
func (t *SelectTable) SetSelectedID(id string) SelectTableI {
	t.selectedID = id
	t.SetAttribute("value", id) // should update this value as needed.
	return t.this()
}
...
func (c SelectTableCreator) Init(ctx context.Context, ctrl SelectTableI) {
	...
	if c.OnRowSelected != nil {
		ctrl.On(RowSelectedEvent().Action(c.OnRowSelected))
	}
...
func GetSelectTable(c page.ControlI, id string) *SelectTable {
	return c.Form().GetControl(id).(*SelectTable)
}

func init() {
	page.RegisterControl(func() page.ControlI { return new(SelectTable) })
}
