...
func (r *Repeater) DrawTag(ctx context.Context, w io.Writer) {
	log.Debug(ctx, logModule, "Drawing repeater tag")
	if r.HasDataProvider() {
		log.Debug(ctx, logModule, "Getting repeater data")
...
func (r *Repeater) Deserialized() {
	r.ControlBase.Deserialized()
	if r.itemHtmlerId != "" {
		r.itemHtmler = r.Form().GetControl(r.itemHtmlerId).(RepeaterHtmler)
	}
	return
}
...
func init() {
	page.RegisterControl(func() page.ControlI { return new(Repeater) })
}
