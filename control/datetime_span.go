...
func (s *DateTimeSpan) DrawingAttributes(ctx context.Context) html5tag.Attributes {
	a := s.ControlBase.DrawingAttributes(ctx)
	a.SetData(page.ControlTypeDataAttribute, "datetimespan")
	return a
}
...
func init() {
	page.RegisterControl(func() page.ControlI { return new(DateTimeSpan) })
}
