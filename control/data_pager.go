...
func (c *PagedControl) SliceOffsets() (start, end int) {
	...
	end = min(start+c.PageSize(), c.TotalItems())
	return
}
...
func (d *DataPager) DrawingAttributes(ctx context.Context) html5tag.Attributes {
	...
	a.SetData(page.ControlTypeDataAttribute, "datapager")
	return a
}
...
func (d *DataPager) CalcBunch() (pageStart, pageEnd int) {
	...
	if pageCount <= d.maxPageButtons {
		...
		minEndOfBunch := min(d.maxPageButtons-2, pageCount)
		maxStartOfBunch := max(pageCount-d.maxPageButtons+3, 1)
		...
		if pageNum < leftBunchTrigger {
			...
			pageStart = min(maxStartOfBunch, pageNum-leftOfBunchCount)
		}

		if pageNum > rightBunchTrigger {
			pageEnd = pageCount
		} else {
			pageEnd = max(minEndOfBunch, pageNum+rightOfBunchCount)
		}
		return
	}
}
...
func (d *DataPager) PagedControl() PagedControlI {
	return d.Form().GetControl(d.pagedControlID).(PagedControlI)
}
...
func (c DataPagerCreator) Create(ctx context.Context, parent page.ControlI) page.ControlI {
	if parent.Form().GetControl(c.PagedControlID) == nil {
		panic("you must declare the paged control before the data pager")
	}
	p := parent.Form().GetControl(c.PagedControlID).(PagedControlI)
...
func init() {
	page.RegisterControl(func() page.ControlI { return new(DataPager) })
}
