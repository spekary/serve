...
func NewNodeColumn(node query.Node) *NodeColumn {
	i := NodeColumn{}
	i.Init(node)
	i.node = node
	return &i
...
func (c *NodeColumn) Init(node query.Node) {
	if node == nil {
...
func (c *NodeColumn) GetNode() query.Node {
	return c.node
}
...
func (c *NodeColumn) CellData(_ context.Context, _ int, _ int, data interface{}) interface{} {
	if v, ok := data.(Getter); !ok {
		...
		for {
			name := query.NodeField(n)
			if name == "" {
				break
			}
			names = append(names, name)
			n = query.NodeParent(n)
			if n == nil {
				break
			}
		}
