...
func (c *FormFieldWrapper) Init(self page.ControlI, parent page.ControlI, id string) {
...
func (c *FormFieldWrapper) DrawTag(ctx context.Context, w io.Writer) {
	log.Debug(ctx, logModule, "Drawing FormFieldWrapper", "id", c.ID())
	...
	if c.forID != "" {
		if child = c.Form().GetControl(c.forID); child != nil {
...
func (c *FormFieldWrapper) checkChildValidation() {
	...
	child := c.Form().GetControl(c.forID)
	if child == nil {
		return
	}
...
func init() {
	page.RegisterControl(func() page.ControlI { return new(FormFieldWrapper) })
}
