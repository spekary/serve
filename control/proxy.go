...
func (p *Proxy) Init(self page.ControlI, parent page.ControlI, id string) {
...
func (p *Proxy) LinkHtml(ctx context.Context,
	...
	if attributes.Has("href") {
		...
		href = page.GetRequest(ctx).URL.RequestURI() // for non-javascript compatibility
...
func (p *Proxy) WrapEvent(eventName string, _ string, eventJs string, options map[string]interface{}) string {
	...
	return fmt.Sprintf(`g$('%s').on('%s', '[data-gr-proxy="%s"]', function(event, eventData){%s}, %s);`, p.ParentControl().ID(), eventName, p.ID(), eventJs, javascript.ToJavaScript(options))
}
...
func init() {
	page.RegisterControl(func() page.ControlI { return new(Proxy) })
