...
//
// HTML defines two basic kinds of textbox controls.
// Single line textboxes are drawn using an "input" tag with a "type" attribute of "text", or one of
// the text-like types, like "password", "search", etc.
// Multiline textboxes are drawn with a textarea tag.
// Use [SetRowCount] to control which kind of control is drawn.
//
// Textboxes can be validated before they are submitted to the form by using [ValidateWith] to
// add a Validater.
//
// You can also set a global sanitizer to attempt to remove malicious or invalid values from the textbox.
// See [config.GlobalSanitizer].
type Textbox struct {
	page.ControlBase
...
// DrawInnerHtml is an internal function that renders the inner html of a tag.
// For a Textbox, it only draws HTML when drawing a textarea tag.
...
//
// The default uses a global sanitizer created at startup.
