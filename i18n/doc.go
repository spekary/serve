...
libraries and the goradd core to provide their own translations and translators of their own strings.
Also, you can replace those and provide your own translations of these same strings if you want.
Library makers, and the goradd framework, simply need to register their translators during their own package
initialization, and you can replace those during the local application package initialization process.
...
To send a string to your application translator, simply call Translate("message") on any control or form,
and it will return the translation. You can add some annotations by adding an i18n.ID() call or i18n.Comment()
call to the call, like so:

	newMessage := ctrl.Translate("my message",
		i18n.ID("my_translation_context_id"),
		i18n.Comment("This can become an extracted comment))
