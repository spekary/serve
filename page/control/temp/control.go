...
import (
	...
	error2 "github.com/goradd/serve/error"
	...
	"github.com/goradd/serve/watcher"
)
...
type ControlI interface {
	...
	removeChild(id string, fromPage bool)
}
...
func (c *Control) Init(self any, parent ControlI, id string) {
	c.Base.Init(self)
	c.attributes = html5tag.NewAttributes()
	if parent == nil {
		c.id = id
	} else {
		c.id = c.ParentForm().GenerateControlID(id)
	}
...
func (c *Control) SetTag(tag string) ControlI {
	c.Tag = tag
	return c
}
...
func (c *Control) Draw(ctx context.Context, w io.Writer) {
...
func (c *Control) DrawAjax(ctx context.Context, response *Response) {
...
func (c *Control) DrawPostRender(ctx context.Context, w io.Writer) {
	// Update watcher
	//if ($This->objWatcher) {
...
func (c *Control) DrawTag(ctx context.Context, w io.Writer) {
	var ctrl string

	log.Debug(ctx, "control", "Drawing tag", "id", c.ID())
...
func (c *Control) RenderAutoControls(ctx context.Context, w io.Writer) {
...
func (c *Control) DrawTemplate(ctx context.Context, w io.Writer) (err error) {
	// Don't change this to use some kind of function injection, as such things are not serializable
	return error2.NewFrameworkError(error2.FrameworkErrNoTemplate)
}
...
func (c *Control) DrawInnerHtml(ctx context.Context, w io.Writer) {
	if err := c.this().DrawTemplate(ctx, w); err == nil {
		return
	} else if appErr, ok := err.(error2.FrameworkError); !ok || appErr.Err != error2.FrameworkErrNoTemplate {
		panic(err)
	}
...
func (c *Control) DrawChildren(ctx context.Context, w io.Writer) {
	for _, child := range c.children {
...
func (c *Control) DrawText(ctx context.Context, w io.Writer) {
	if c.text != "" {
		text := c.text
...
func (c *Control) DrawingAttributes(ctx context.Context) html5tag.Attributes {
	a := c.attributes.Copy()
...
func (c *Control) MergeAttributes(a html5tag.Attributes) ControlI {
...
func (c *Control) ProcessAttributeString(s string) ControlI {
	if s != "" {
...
func (c *Control) HasClass(class string) bool {
...
func (c *Control) Attributes() html5tag.Attributes {
	return c.attributes
}
...
func (c *Control) Remove() {
	if c.parent != nil {
		c.parent.removeChild(c.this().ID(), true)
	} else {
		c.ParentForm().removeControl(c.this().ID())
	}
}
...
func (c *Control) removeChild(id string, fromPage bool) {
	for i, v := range c.children {
		if v.ID() == id {
			...
			if fromPage {
				v.control().removeChildrenFromForm()
				c.ParentForm().removeControl(id)
			}
...
func (c *Control) removeChildrenFromForm() {
	c.RangeAllChildren(func(child ControlI) {
		c.ParentForm().removeControl(child.ID())
	})
}
...
func (c *Control) RemoveChildren() {
	for _, child := range c.children {
		child.control().removeChildrenFromForm()
		c.ParentForm().removeControl(child.ID())
...
func (c *Control) SetParent(newParent ControlI) {
	...
	c.ParentForm().addControl(c.this())
...
func (c *Control) On(e *event.Event, a ...action.ActionI) ControlI {
...
func (c *Control) Off(ids ...event.EventID) {
	if ids == nil {
...
// call this normally.
func (c *Control) PrivateOff(ids ...event.EventID) {
	if ids == nil {
		for id, e := range c.events {
			if event.IsPrivate(e) {
				delete(c.events, id)
...
func (c *Control) Event(eventName string) *event.Event {
...
// object hierarchy.
func (c *Control) DoAction(ctx context.Context, a action.Params) {
...
func (c *Control) DoPrivateAction(ctx context.Context, a action.Params) {
}
...
// UpdateFormValues is used by the framework to cause the control to retrieve its values from the form
func (c *Control) UpdateFormValues(ctx context.Context) {
...
func (c *Control) doAction(ctx context.Context) {
	...
	var ok bool
	var isPrivate bool
	var request = page.GetRequest(ctx)
	...
	if c.passesValidation(ctx, e) {
		log.Debug(ctx, "control", "doAction - triggered event", "event", e.String())
		if callbackAction := event.GetCallbackAction(e); callbackAction != nil {
			...
			if c.ParentForm().HasControl(controlId) {
				dest := c.ParentForm().GetControl(controlId)
				if isPrivate {
						log.Debug(ctx, "control", "doAction - DoPrivateAction", DestId: %s, ActionId: %d, DoAction: %s, TriggerId: %s",
...
func (c *Control) SetValidationType(typ event.ValidationType) ControlI {
...
func (c *Control) ValidationType(e *event.Event) event.ValidationType {
...
func (c *Control) passesValidation(ctx context.Context, e *event.Event) (valid bool) {
...
func (c *Control) validateSelfAndSiblings(ctx context.Context) bool {
...
func (c *Control) validateSelfAndChildren(ctx context.Context) bool {
	if !c.IsOnPage() {
		return true
	}
...
func (c *Control) MarshalState(m SavedState) {
}
...
func (c *Control) UnmarshalState(m SavedState) {
}

// GT translates strings using the Goradd dictionary.
func (c *Control) GT(message string) string {
...
func (c *Control) SetStyles(s html5tag.Style) {
...
// The default calls the data connector to do this.
func (c *Control) ModifiesData(data interface{}) bool {
	if c.dataConnector != nil && c.IsOnPage() {
...
// own serializer.
func (c *Control) Serialize(e Encoder) {
	s := controlEncoding{
		Id:                    c.id,
		Tag:                   c.Tag,
...
func (c *Control) Deserialize(d Decoder) {
...
func (c *Control) AddControls(ctx context.Context, creators ...Creator) {
...
func (c *Control) FireTestMarker(marker string) {
	if config.Debug {
